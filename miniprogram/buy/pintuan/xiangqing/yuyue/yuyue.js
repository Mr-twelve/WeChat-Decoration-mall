...
Page({
  ...
  onLoad: function(options) {
    ...
    this.setData({
      ...
      pintuanstyle: options.pintuanstyle,
      peoplestyle: options.peoplestyle,
    ...
    db.collection('worker').doc(options.id).get({
      success: function(res) {
        self.setData({
          pintuanpeople: res.data.pintuanpeople,
          workprice: res.data.pintuanxiangqin,
  ...
  formSubmit(e) {
    ...
    var pintuanstyle = this.data.pintuanstyle
    var peoplestyle = this.data.peoplestyle
    var pintuanpeople = this.data.pintuanpeople
                        ...
                        if (fileIDList.length == self.data.imgList.length) {
                          ...
                          db.collection('yuyue').add({
                            data: {
                              ...
                              jindu: 999, //进度1为付款1元，50为完成50%，100为完工
                            ...
                            success: function(ress) {
                              ...
                              if (parseInt(peoplestyle) == 999) {
                                ...
                                var newpintuanpeople = pintuanpeople.concat(newarr)
                                console.log(newarr)
                                wx.cloud.callFunction({
                                  name: 'yuyue-add-worker-pintuan',
                                  data: {
                                    workerid: workerid,
                                    pintuanpeople: newpintuanpeople
                                  },
                                  success: res => {
                                    wx.showToast({
                                      title: '预约发布成功',
                                ...
                                var newarrrr =[]
                                pintuanpeople.forEach((value, index) => {
                                  console.log(value)
                                  if (index == parseInt(peoplestyle)){
                                    var newpeople = parseInt(value.people) + 1
                                    var newvalue = {
                                      style: yuyuename,
                                      people: newpeople,
                                      user: value.user.concat(peoplearr)
                                    }
                                    if (newpeople == parseInt(pintuanstyle)){
                                      maxpeople=true
                                      newuserid.push(ress._id)
                                      value.user.forEach((value, index) => {
                                        newuserid.push(value.id)
                                      })
                                    }else{
                                      newarrrr.push(newvalue)
                                    }
                                  }else{
                                    newarrrr.push(value)
                                  }
                                ...
                                wx.cloud.callFunction({
                                  ...
                                  success: res => {
                                    if (maxpeople==false){
                                      ...
                                      for (var i = 0; i <= parseInt(pintuanstyle);i++){
                                        wx.cloud.callFunction({
                                          name: 'yuyue-pintuan-true',
                                          data: {
                                            id: newuserid[i]
                                          },
                                        })
                                        if (i == parseInt(pintuanstyle)){
                                          wx.showToast({
                                            title: '预约发布成功',
                                          });
                                          setTimeout(function () {
  ...
  noknow(e) {
    this.setData({
      totalprice: '未测量将以实际施工为标准并附加300订金',
      sumhaveprice: null,
      sumnoprice: null,
      noknow: true,
      tabpage: 0,
