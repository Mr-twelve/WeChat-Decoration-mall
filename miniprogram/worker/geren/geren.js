...
Page({
    ...
    db.collection('worker').where({
      ...
      success: function (res) {
        ...
        if (res.data.length != 0) {
          if (res.data[0].region2!=null){
            self.setData({
              addregion2:true,
              region2: res.data[0].region2,
            })
          }
          self.setData({
            imgList: [],
  ...
  ViewImage(e) {
    console.log(e)
    console.log(this.data.worker.workerjinenglook)
    wx.previewImage({
      urls: e.currentTarget.id,
      current: this.data.worker.workerjinenglook
    });
  },
  DelImg(e) {
  ...
  shezhifuwufanwei1(e) {
    ...
    var region2 = this.data.region2
    var addregion2 = this.data.addregion2
    if (addregion2==false){
      db.collection('worker').doc(id).update({
        data: {
          region: region,
  ...
  gongjiachakan(e) {
    var id = this.data.id
    var workerprice = this.data.worker.workprice
    console.log(id)
    wx.navigateTo({
      url: 'workerprice/workerprice?workerprice=' + workerprice,
      success: function (res) {
        // 通过eventChannel向被打开页面传送数据
        res.eventChannel.emit('acceptDataFromOpenerPage', { data: workerprice })
      }
    })
  },
  work(e){
  ...
  pintuan(e) {
    wx.navigateTo({
      url: 'pintuan/pintuan?id='+this.data.id,
    })
  },
  addquyu(e){
    this.setData({
      addregion2: !this.data.addregion2
    })
  },
  shezhijineng(e){
    wx.navigateTo({
      url: 'shezhijineng/shezhijineng?id=' + this.data.id,
    })
  }
})